---
title: "A/B subcompartments"
output: rmarkdown::github_document
---

```{r, echo = FALSE}
knitr::opts_chunk$set(
  fig.path = "figures/hic-"
)
```
...
## Plot
```{r, SupFig7A, fig.height=4, fig.width=5, message=FALSE, warning=FALSE}
clustering_cor_plot <- 
...
```{r, SupFig7C, fig.height=6, fig.width=5, message=FALSE, warning=FALSE}
...
## Plot Norm. Counts 
```{r, Fig3A, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
figure_3A <-
  leaf_roi_obs %>%
  mutate(tissue = 'leaf') %>%
...
To see the higher degree of packing in the root subcluster in shoot tissue.

Juicebox calculates first the average counts of the whole normalized Hi-C map on chromosome 1 and divides the normalized counts on each bin by this mean. Then, it divides your observed (root) / control (leaf) Hi-C maps.
...
#### Plot 
```{r, Fig3B, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
figure_3B <-
...
We specifically want to look at the region 108-118Mb in chromosome 1. We expect that the whole cluster will loop out of the neighbouring region
...
```{r, SupFig7D, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
leaf_roi_obs_ab_compartment %>%
...
```{r, Figure3C_heatmap, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
cor_leaf <- 
  leaf_roi_oe_ab_compartment %>%
...
```{r, Figure3C_pca, fig.height=2, fig.width=5, message=FALSE, warning=FALSE}
leaf_pca_roi <- 
  leaf_roi_oe_ab_compartment %>%
  rbind(data_frame(x = leaf_roi_oe_ab_compartment$y, 
                   y = leaf_roi_oe_ab_compartment$x, 
...
```{r, Figure3C, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
...
```{r, SupFig7E, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
...
```{r, Figure3D_heatmap, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
cor_root <- 
  root_roi_oe_ab_compartment %>%
...
```{r, Figure3D_pca, fig.height=2, fig.width=5, message=FALSE, warning=FALSE}
pca_roi_root <- 
  root_roi_oe_ab_compartment %>%
...
```{r, Figure3D, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
